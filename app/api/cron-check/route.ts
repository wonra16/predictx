...
import { getPendingResults, getUserStats, updatePrediction, updateUserStats } from '@/lib/simple-storage';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const now = Date.now();
    const pendingPredictions = await getPendingResults(100);
